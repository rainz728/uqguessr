  ...
  ws.on("message", (raw) => {
    try {
      ...
      if (
        ...
        throw new Error(
          "Client ID cannot change during a connection."
        );
      }
      ...
      if (msg.type === "leave_game") {
        if (isHost) {
          fail(
            ws,
            "The host must use End game to close the room."
          );
      ...
      if (msg.type === "end_game") {
        ...
        closeRoom(
          game,
          "The host ended the game. You can join a new room now."
        );
...
console.log(
  `UQGuessr WebSocket server running on ws://0.0.0.0:${port}`
);